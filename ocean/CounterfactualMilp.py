...
class CounterfactualMilp:
    ...
    def __init__(self, sample, gurobiEnv, objectiveNorm=2, verbose=False,
        ...
        if verbose:
            gurobiEnv.setParam('OutputFlag', 1)
        else:
            gurobiEnv.setParam('OutputFlag', 0)
        self.model = gp.Model("CounterFactualMilp", env=gurobiEnv)
    ...
    def __read_feature_types(self, featuresType):
        ...
        if not featuresType:
            self.featuresType = [FeatureType.Numeric for f in features]
        else:
            self.featuresType = featuresType
        # Read types of features
        self.binaryFeatures = [
            f for f in features if self.featuresType[f] == FeatureType.Binary]
        ...
        self.continuousFeatures = [
            f for f in features if self.featuresType[f] == FeatureType.Numeric]
        self.discreteFeatures = [
            f for f in features if self.featuresType[f] == FeatureType.Discrete]
        ...
        self.discreteAndCategoricalNonOneHotFeatures = [
            f for f in features
            if self.featuresType[f] in [FeatureType.CategoricalNonOneHot,
    ...
    def read_results(self):
        ...
        if self.model.status != GRB.OPTIMAL:
            ...
            return False
        # --- Get model solution ---
        # Read counterfactual distance
        self.objValue = self.model.ObjVal
        # Read counterfactual explanation-
        self.x_sol = [[]]
        for f in range(self.nFeatures):
